...
class Forecast:
    ...
    def _octopus_results_to_series(
        ...
        required_columns = {"valid_from", "valid_to", "value_inc_vat"}
        ...
        rates_df["valid_to"] = pd.to_datetime(rates_df["valid_to"], utc=True)
        rates_df = rates_df.sort_values("valid_from")
        rates_df["valid_from"] = rates_df["valid_from"].dt.tz_convert(self.time_zone)
        rates_df["valid_to"] = rates_df["valid_to"].dt.tz_convert(self.time_zone)

        target = pd.DatetimeIndex(target_index)
        if target.tz is None:
            target = target.tz_localize(self.time_zone)
        else:
            target = target.tz_convert(self.time_zone)
        if target.empty:
            return pd.Series(dtype=float, index=target)

        coverage_start = rates_df["valid_from"].min()
        coverage_end = rates_df["valid_to"].max()
        if target.min() < coverage_start or target.max() >= coverage_end:
            raise ValueError(
                "Octopus tariff data does not fully cover forecast horizon. "
                f"Coverage [{coverage_start}, {coverage_end}) vs forecast "
                f"[{target.min()}, {target.max()}]."
            )

        # Use interval membership to prevent extrapolating values outside returned API ranges.
        rates_df = rates_df.assign(
            value=rates_df["value_inc_vat"].astype(float) / 100.0,  # pence/kWh -> currency unit/kWh
            interval=pd.IntervalIndex.from_arrays(
                rates_df["valid_from"], rates_df["valid_to"], closed="left"
            ),
        )
        interval_index = rates_df["interval"]
        indexer = interval_index.get_indexer(target)
        if np.any(indexer < 0):
            raise ValueError(
                "Unable to align Octopus tariff data with forecast timestamps. "
                "Tariff data has gaps or does not match forecast granularity."
            )
        forecast_series = pd.Series(
            rates_df["value"].to_numpy()[indexer],
            index=target_index,
        )
